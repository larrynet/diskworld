 ...
 * @author Niloufar
 * 
 *         Class ManageCards use to hold the attributes of every single card in
 *         the game. These Cards will be initiated at the beginning of the game.
 *
 */
...
public class ManageCards implements Serializable {
	 ...
	 */
	public ManageCards() {
		String[] PersonalityName = { "Lord Vetinari", "Lord Selachii ",
				"Lord Rust ", "Lord de Word ", "Dragon King of Arms ",
				"Chrysoprase ", "Commander Vimes" };
		String[] EventName = { "The Dragon", "Flood", "Fire", "Fog", "Riots",
				"Explorion", "Mysterious Murders",
				"Demons From The Dungeon Dimension", "Subsidence",
				"Bloody Stupid Johnson", "Trolls", "Earthquake" };
		String[] CityAreaName = { "Dolly Sisters", "Unreal Estate",
				"Dragon's Landing", "Small Gods", "The Scours", "The Hippo",
				"The Shades", "Dimwell", "Longwall", "Isle of Gods",
				"Seven Sleepers", "Nap Hill" };

		int[] PersonalityId = { 101, 102, 103, 104, 105, 106, 107 };
		int[] EventId = { 201, 202, 203, 204, 205, 206, 207, 208, 209, 210,
				211, 212 };
		int[] CityAreaId = { 301, 302, 303, 304, 305, 306, 307, 308, 309, 310,
				311, 312 };

		Boolean Status = true;
		/*
		 * Make an array of c with 5 card size to point to each five of cards
		 * type
		 * 
		 * When i is zero an arraye of seven personality type card will be
		 * initiated an so the rest.
		 */
		...
		for (int i = 0; i <= 4; i++) {
			...
			if (i == 0) {
				...
				{
					this.p[j] = new PersonalityCards(PersonalityName[j],
							PersonalityId[j], Status, CardType.PersonalityCards);
				...
				for (int j = 0; j <= this.e.length - 1; j++) {
					this.e[j] = new EventCards(EventName[j], EventId[j],
							Status, CardType.EventCards);
				...
				for (int j = 0; j <= this.ct.length - 1; j++) {
					this.ct[j] = new CityAreaCards(CityAreaName[j],
							CityAreaId[j], Status, CardType.CityAreaCards);
				...
				for (int j = 0; j <= this.g.length - 1; j++) {
					this.g[j] = new GreenCards(("Green" + " " + (j + 1)),
							401 + j, Status, CardType.GreenCards);
				...
				for (int j = 0; j <= this.b.length - 1; j++) {
					this.b[j] = new BrownCards(("Brown" + " " + (j + 1)),
							(501 + j), Status, CardType.BrownCards);
	...
	/*
	 * Initiate the Cards and copy Cards value to the CardManager for further
	 * changes
	 */
	public void CardInitiate() {

		String[] PersonalityName = { "PersonalityRole 1", "PersonalityRole 2",
				"PersonalityRole 3", "PersonalityRole 4", "PersonalityRole 5",
				"PersonalityRole 6", "PersonalityRole 7" };
		String[] EventName = { "Event 1", "Event 2", "Event 3", "Event 4",
				"Event 5", "Event 6", "Event 7", "Event 8", "Event 9",
				"Event 10", "Event 11", "Event 12" };
		String[] CityAreaName = { "CityArea 1", "CityArea 2", "CityArea 3",
				"CityArea 4", "CityArea 5", "CityArea 6", "CityArea",
				"CityArea 8", "CityArea 9", "CityArea 10", "CityArea 11",
				"CityArea 12" };
	...
	/**
	 * GetState is used to show the current state of Cards Used to test the
	 * value of cards in built 1 of project.
	 */
	...
	/**
	 * 
	 * Method GetCard will be call from GameEngin to Draw cards to player during
	 * the game.
	 * 
	 * @param Type
	 *            Type of cards which would be personality,event...
	...
	/**
	 * Shuffle method was written shuffle the cards before giving them to
	 * players.
	 * 
	 * @param c
	 *            This parameter is array of cards
